    ...
    <div className="absolute inset-0 bg-background flex items-center justify-center p-4">
      ...
      >
        ...
        {mode === "choose" && (
          <div className="flex flex-col gap-4 mt-4">
            <div className="flex flex-col gap-2">
              <label className="text-primary/70 uppercase text-sm font-mono">Votre Pseudo</label>
              <input
                type="text"
                value={name}
            ...
            <button
              ...
              className="mt-4 px-6 py-4 border border-primary text-primary hover:bg-primary/20 uppercase tracking-widest transition-colors font-mono"
            >
              CRÉER UNE PARTIE
            </button>
            <button
              onClick={() => name.trim() ? setMode("join") : setError("Entrez un pseudo")}
              className="px-6 py-4 border border-primary text-primary hover:bg-primary/20 uppercase tracking-widest transition-colors font-mono"
            >
              REJOINDRE UNE PARTIE
            </button>
            {error && <div className="text-destructive text-sm text-center font-mono">{error}</div>}

            <Link href="/" className="mt-4 text-center text-primary/50 hover:text-primary text-sm uppercase font-mono">
              RETOUR
            </Link>
          </div>
        )}
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                className="bg-black/50 border border-primary/30 p-3 text-primary outline-none uppercase font-mono"
              >
            ...
            <div className="flex flex-col gap-2">
              <label className="text-primary/70 uppercase text-sm font-mono">Joueurs Max ({maxPlayers})</label>
              <input
                type="range"
                min="2" max="8"
                value={maxPlayers}
        ...
        {mode === "join" && (
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-primary/70 uppercase text-sm font-mono">Code du Salon (6 lettres)</label>
              <input
                type="text"
                value={joinCode}
                onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
                className="bg-black/50 border border-primary/30 p-3 text-primary text-center text-2xl tracking-[0.5em] outline-none focus:border-primary uppercase font-mono"
                maxLength={6}
              />
            </div>
            {error && <div className="text-destructive text-sm text-center font-mono">{error}</div>}
